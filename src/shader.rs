...
#[allow(dead_code)]
impl Shader {
    pub fn get_shader_code(shaderPath: &str) -> String{
        let mut shaderFile = File::open(shaderPath)
            .unwrap_or_else(|_| panic!("Failed to open {}", shaderPath));
        let mut shaderCode = String::new();
        shaderFile
            .read_to_string(&mut shaderCode)
            .expect("Failed to read vertex shader");

        shaderCode
    }

    pub fn compute(computePath: &str) -> Shader{
        let mut shader = Shader { ID: 0 };
        // 1. retrieve the vertex/fragment source code from filesystem
        let mut cShaderFile = File::open(computePath)
            .unwrap_or_else(|_| panic!("Failed to open {}", computePath));
        let mut computeCode = String::new();
        cShaderFile
            .read_to_string(&mut computeCode)
            .expect("Failed to read vertex shader");
        let cShaderCode = CString::new(computeCode.as_bytes()).unwrap();

        // 2. compile shaders
        unsafe {
            // compute shader
            let compute = gl::CreateShader(gl::COMPUTE_SHADER);
            gl::ShaderSource(compute, 1, &cShaderCode.as_ptr(), ptr::null());
